...
-- Create processor_batches table
...
-- Create processor_documents table
...
-- RLS Policies for processor_batches

-- Processors can view their own processing batches
...
-- Processors can delete their own processing batches
CREATE POLICY "Processors can delete own processing batches"
  ON processor_batches FOR DELETE
  TO authenticated
  USING (auth.uid() = processor_id);

-- RLS Policies for processor_documents

-- Users can view documents for processing batches they have access to
...
-- Processors can insert documents for their own processing batches
...
-- Processors can delete documents for their own processing batches
...
-- Trigger to auto-update updated_at for processor_batches
